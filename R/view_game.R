...
view_game_shiny <- function(game) {
	...
	assert_suggested("shiny")

	app_dir <- tempfile()
	on.exit(unlink(app_dir))
	old_dir <- system.file("shiny/ppn_viewer", package = "ppn")
