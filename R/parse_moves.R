...
parse_moves <- function(text, df = NULL, state = create_state(df)) {
	...
	if (anyDuplicated(names(dfs))) {
		warning("Non-unique MoveNumbers")
	}
	names(moves) <- names(dfs)
	names(comments) <- names(dfs)
	for (i in seq_along(dfs)) {
		attr(dfs[[i]], "scale_factor") <- state$scale_factor
	}
...
exp_braces <- function(string) {
	if (str_sub(string, 1L, 1L) == "{" && str_sub(string, -1L, -1L) == "}") {
		# no preamble/postfix
		string
	} else {
...
complete_piece <- function(df, std_piece_spec) {
	...
	if (is.na(df$piece)) {
		df$piece <- if (df$side %in% c("left", "right", "top")) {
			"pyramid"
		} else if (str_detect(simple, "[RKGBYW]")) {
			"bit"
	...
	if (is.na(df$cfg)) {
		if (str_detect(simple, "[RKGBYW]")) {
			df$cfg <- switch(
				df$piece,
				bit = switch(
					str_extract(simple, "[smc]"),
					s = "go",
					c = getOption("ppn.default_checker_cfg", "checkers2"),
					m = "meeples",
					getOption(
						"ppn.default_colored_bit_cfg",
						abort(
							str_glue("Don't know proper cfg for piece '{simple}'"),
							class = "infer_piece"
						)
					)
				),
				...
				abort(str_glue("Don't know proper cfg for piece '{simple}'"), class = "infer_piece")
			)
	...
	if (str_detect(simple, "u")) {
		df$cfg <- switch(
			...
			checkers2 = "checkers1",
			piecepack = "subpack",
			abort(paste("Don't know miniature version of cfg", df$cfg), class = "infer_piece")
		)
	}
	if (is.na(df$side)) {
		df$side <- switch(
	...
	if (is.na(df$suit)) {
		df$suit <- switch(df$cfg, go = 2L, checkers1 = 3L, checkers2 = 3L, dice = 6L, 1L)
	}
	if (is.na(df$rank)) {
		if (df$piece == "board") {
			df$rank <- switch(df$cfg, go = 19L, marbles = 4L, 8L)
		} else if (df$piece == "bit" && df$cfg == "marbles") {
			df$rank <- 9L
		} else {
			df$rank <- 1L
		}
...
get_id_from_piece_id <- function(piece_id, df, state = create_state(df)) {
	...
	if (piece_id == "") {
		id <- state$active_id
		if (!length(id)) {
			abort("Couldn't find any active pieces", class = "identify_piece")
		}
		id
...
process_rotate_move <- function(df, text, state = create_state(df), clockwise = TRUE) {
	...
	if (!is.null(location)) {
		...
		p$translate(-location)$rotate(affiner::angle(angle, "degrees"))$translate(location)
		df$x[indices] <- p$x
		df$y[indices] <- p$y
	}
...
process_plus_move <- function(df, text, state) {
	...
	df$rank[indices] <- ifelse(
		df$piece_side[indices] == "die_face",
...
process_underscore_percent_move <- function(df, text, state) {
	# nolint
...
greedy_match <- function(df, piece_spec) {
	...
	with_incomplete <- which(
		ngm_helper(dfi$piece, str_detect(df$piece_side, paste0("^", dfi$piece))) &
...
non_greedy_match <- function(df, piece_spec) {
	...
	if (length(with_incomplete) == 1) {
		return(df$id[with_incomplete])
	}
	dff <- complete_piece(dfi, std_piece_spec)
	with_angle <- which(
		df$piece_side == dff$piece_side &
			ngm_helper(dff$suit, df$suit == dff$suit) &
			ngm_helper(dff$rank, df$rank == dff$rank) &
			df$cfg == dff$cfg &
			near(df$angle, dff$angle)
	)
	if (length(with_angle)) {
		return(df$id[tail(with_angle, 1L)])
	}
	without_angle <- which(
		df$piece_side == dff$piece_side &
			ngm_helper(dff$suit, df$suit == dff$suit) &
	...
	if (length(without_angle)) {
		return(df$id[tail(without_angle, 1L)])
	}
...
process_tilde_move <- function(df, text, state = create_state(df)) {
	...
	if (!is.na(dfp$rank)) {
		# if change rank 1L if die_face and not 2L already else 2L
		to_change <- is.na(df$rank[indices]) ||
			df$rank[indices] !=
				ifelse(df$cfg[indices] %in% index_rank_by_one, dfp$rank - 1L, dfp$rank)
		to_change_id[to_change] <- ifelse(
			df$piece_side[indices[to_change]] == "die_face" &&
				to_change_id[to_change] < 2L,
			1L,
			2L
		)
...
get_xy <- function(coords, df, state = create_state(tibble()), anchor_indices = NULL) {
	xy <- if (coords == "") {
		...
		if (is.null(location)) {
			...
			affiner::as_coord2d(x = xy[1] + df$x[anchor_indices], y = xy[2] + df$y[anchor_indices])
		} else {
			affiner::as_coord2d(x = xy[1] + location$x, y = xy[2] + location$y)
		}
	} else if (str_detect(coords, "^[0-9]")) {
		# alternative relative moves
		coords <- convert_relative(coords)
		get_xy(coords, df, state, anchor_indices)
	} else {
		p <- affiner::as_coord2d(x = get_x(coords), y = get_y(coords))$scale(state$scale_factor)
	}
...
convert_relative_helper <- function(coords) {
	...
	multiplier <- switch(
		...
		UUR = hex_xy(60, number),
		RUR = hex_xy(30, number),
		RDR = hex_xy(330, number),
		DDR = hex_xy(300, number),
		DDL = hex_xy(240, number),
		LDL = hex_xy(210, number),
		LUL = hex_xy(150, number),
		UUL = hex_xy(120, number),
		abort(str_glue("Don't know direction {direction}"), class = "infer_location")
	)
...
process_moves <- function(df, movelist, state = create_state(df)) {
	...
	nms[1] <- "SetupFn."
	if (!is.null(names(movelist))) {
		nms[seq_along(movelist) + 1L] <- names(movelist)
	}
	...
	for (i in seq_along(movelist)) {
		df <- tryCatch(process_move(df, movelist[[i]], state = state), error = function(e) {
			msg <- c(
				str_glue(
					"Couldn't process move `{movenumber} {move}`",
					movenumber = nms[i + 1L],
					move = movelist[[i]]
				),
				i = e$message
