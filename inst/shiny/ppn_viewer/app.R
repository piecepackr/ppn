...
ui <- fluidPage(
	...
	fluidRow(column(5, gameUI("game", txt), hr(), moveUI("move")), column(7, plotUI("plot")))
)

server <- function(input, output, session) {
	if (!has_animation) {
		showNotification(
			"Neither 'animation' or 'gifski' packages installed.  GIF animation disabled.",
			type = "warning"
		)
	}
	...
	if (!has_fansi) {
		showNotification(
			"`{fansi}` package not installed. `cat_piece()`'s `color` option disabled.",
			type = "warning"
		)
	}
	if (!has_piecenikr) {
		showNotification(
			"`{piecenickr}` package not installed.  `icehouse_pieces` support disabled.",
			type = "warning"
		)
	}
	if (!has_rgl) {
		showNotification("`{rgl}` package not installed. webGL support disabled.", type = "warning")
	}
	if (!has_tweenr) {
		showNotification(
			"'tweenr' package not installed.  Animation transitions disabled.",
			type = "warning"
		)
	}
	...
	if (!has_dejavu) {
		showNotification(
			'"Dejavu Sans" font not detected.  Falling back to "sans" style.',
			type = "warning"
		)
	}
