...
gameUI <- function(id, ppn_text = "") {
	...
	tagList(
		...
		ppn_link,
		"for a game:",
		textAreaInput(ns("ppn_text"), "", ppn_text, width = "100%", rows = 24),
...
gameServer <- function(id) {
	moduleServer(id, function(input, output, session) {
		game <- eventReactive(input$parse_ppn, {
			op <- options()
			on.exit(options(op))
			options(cli.num_colors = 1L) # prevent adding ANSI Control codes to error message
			ppn <- textConnection(input$ppn_text)
			try(ppn::read_ppn(ppn)[[1]])
		})

		observe({
			query <- parseQueryString(session$clientData$url_search)
			if (!is.null(query$system.file)) {
				package <- query$package %||% "ppn"
				...
				if (file.exists(f)) {
					...
					msg <- str_glue(
						"system.file '{query$system.file}' does not exist in package '{package}'"
					)
...
moveUI <- function(id) {
	ns <- NS(id)
	tagList(
		selectInput(ns("move"), "Select move", character(0)),
		fluidRow(
			column(2, actionButton(ns("prev_move"), "Prev.")),
...
plotUI <- function(id) {
	ns <- NS(id)
	tagList(selectInput(ns("renderer"), "Select renderer", renderers), uiOutput(ns("plot")))
}
...
gridUI <- function(id) {
	...
	if (has_animation) {
		if (has_tweenr) {
			animation_ui <- tagList(
				hr(),
				fluidRow(
					column(3, actionButton(ns("animate"), "Create GIF!")),
					column(
						3,
						numericInput(ns("n_transitions"), "n_transitions", 0, min = 0, step = 1)
					),
			...
			animation_ui <- tagList(
				...
				conditionalPanel(
					'false',
					column(1, numericInput(ns("n_pauses"), "n_pauses", 1, min = 1, step = 1))
				)
			)
		}
	} else {
	...
	renderUI(tagList(
		fluidRow(
			column(3, numericInput(ns("op_angle"), "op_angle", 90, step = 15)),
	...
	moduleServer(id, function(input, output, session) {
		...
		output$grid <- renderImage(
			{
				if (v$should_animate) {
					...
					id <- showNotification(
						"I'm drawing animation frames as fast as I can.  Please be patient.",
						type = "message",
						duration = NULL,
						closeButton = FALSE
					)
					...
					id <- showNotification(
						"I'm now painting the game diagram.  Please be patient.",
						type = "message",
						duration = NULL,
						closeButton = FALSE
					)
		...
		observe({
			...
			if (!is.null(query$op_angle)) {
				...
				if (is.na(opa)) {
					showNotification(
						"Bad 'op_angle' URL parameter value. Ignoring.",
						type = "warning"
					)
				} else {
			...
			if (!is.null(query$op_scale)) {
				...
				if (is.na(ops) || ops < 0) {
					showNotification(
						"Bad 'op_scale' URL parameter value. Ignoring.",
						type = "warning"
					)
				} else {
			...
			if (!is.null(query$annotate)) {
				if (query$annotate %in% c("none", "cartesian", "algebraic")) {
					...
					showNotification(
						"Bad 'annotate' URL parameter value. Ignoring.",
						type = "warning"
					)
				}
			}
...
rglServer <- function(id, game, move) {
	moduleServer(id, function(input, output, session) {
		output$rgl <- rgl::renderRglwidget({
			req(game(), move())
			id <- showNotification(
				"Crafting rotatable, zoomable 3D model.  Please be patient.",
				type = "message",
				duration = NULL,
				closeButton = FALSE
			)
