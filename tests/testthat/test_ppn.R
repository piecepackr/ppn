...
test_that("parsing ppn files works as expected", {
	...
	ppn_warning <- "---\n...\n1. S@b2 1. b2-b3"
	expect_warning(read_ppn(textConnection(ppn_warning)), "Non-unique MoveNumbers")
	...
	ppn_bad_metadata <- "---\nttt\n...\n"
	expect_error(
		read_ppn(textConnection(ppn_bad_metadata)),
...
test_that("parsing simplified piece notation works as expected", {
	...
	expect_error(process_move(df, "`boobear'@b4"), "Macro boobear is unknown")
...
test_that("ID computation works as expected", {
	...
	expect_equal(
		compute_plus_id(c("23", "24.2", "24..1.1", "30..2")),
		c("23.1", "24.3", "24..1.2", "30..2.1")
	)
	expect_equal(
		compute_equal_id(c("23", "24.2", "24..1.1", "30..2")),
